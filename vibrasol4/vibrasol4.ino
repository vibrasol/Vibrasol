 ...
 * 3.  Output 13 - Vibrator motor (flat coin type, in contact with skin, 5V) - GND
 ...
 Change Log:
 ********** ON ARDUINO NANO MOTOR IS ON PORT D2 ********** 
...
int motorPin    = 2;     // select the pin for the LED
...
void logit(){
    ...
    // |- last @ written (2Byte) -|- userid (2Byte) -|- totalbadcount_1 -|- ..._2 -|- ... -|1023
    /*
    if (!EEPROM_FULL) {
       write totalbadcount to EEPROM
    else {  
       // notify user to go back to lab
      vibrate();vibrate();vibrate();vibrate();vibrate();vibrate();vibrate()       
    }
    // if reset button is pushed erase EEPROM ? can do?    
  */   
...
void sample() {
  ...
  Serial.println( p );
  // Record to EEPROM every 15 minutes -------------
  if (elapsed*sampling_t > (15*60*1000-1) ){
    elapsed = 0;
    logit();
  }
  // --- END EEPROM -------------
